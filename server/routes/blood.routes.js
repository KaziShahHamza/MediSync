...
router.get("/donors", async (req, res) => {
  try {
    ...
    const publicDonors = donors.map((donor) => ({
      ...
      district:
        donor.location?.district || "",

      upazila:
        donor.location?.upazila || "",
...
async function checkAndUpdateRateLimit({
  ...
  const key = hashValue(
    `${ipHash}:${deviceId || "no-device"}`,
  );

  let rateLimit =
    await BloodRequestRateLimit.findOne({
      key,
    });
  ...
  if (!rateLimit || rateLimit.expiresAt <= now) {
    rateLimit =
      await BloodRequestRateLimit.findOneAndUpdate(
        { key },
        {
          $set: {
            count: 1,
...
function validateBloodRequest(body) {
  const bloodGroup = normalizeString(
    body.bloodGroup,
  );
  ...
  const hospitalAddress = normalizeString(
  ...
  const requesterName = normalizeString(
    body.requesterName,
  );
  ...
  try {
    ...
    const request =
      await BloodRequest.create({
        ...
        location: {
          district:
            normalizeString(district),

          upazila:
            normalizeString(upazila),
        },

        hospital: {
          name:
            normalizeString(hospitalName),

          address:
            normalizeString(hospitalAddress),
        },
    ...
    try {
      ...
      if (!authorized) {
        return res.status(403).json({
          message:
            "You are not authorized to modify this blood request.",
        });
      }
      ...
      request.bagsNeeded =
        Number(req.body.bagsNeeded);

      request.compensationOffered =
        req.body.compensationOffered;
