// server/services/pdf/sections/doctorsSection.js
...
export function renderDoctorsSection(doc, doctors = []) {
  ...
  doctors.forEach((doctor, index) => {
    ...
    doc
      ...
      .text(
        `${index + 1}. ${doctor.name || "Unnamed Doctor"}`,
        x,
        y,
        {
          width,
    ...
    if (doctor.bmdcRegNo) {
      details.push(`BMDC Reg. No: ${doctor.bmdcRegNo}`);
    }

    if (doctor.designation) {
      details.push(`Designation: ${doctor.designation}`);
    }
    ...
    if (doctor.primaryHospital) {
      details.push(
        `Primary Hospital: ${doctor.primaryHospital}`,
      );
    }
    ...
    if (doctor.chambers?.length) {
      ...
      doctor.chambers.forEach((chamber, chamberIndex) => {
        doc
          ...
          .fillColor(PDF_COLORS.textSecondary)
          .text(
            `${chamberIndex + 1}. ${chamber.name || "Chamber"}`,
            x + 20,
          );

        if (chamber.address) {
          doc.text(`Address: ${chamber.address}`, x + 30);
        }

        if (chamber.phone) {
          doc.text(`Phone: ${chamber.phone}`, x + 30);
        }
        ...
        if (chamber.visitingTime) {
          doc.text(
            `Visiting Time: ${chamber.visitingTime}`,
            x + 30,
          );
        }
      });
    ...
    if (doctor.notes) {
      ...
      doc
        ...
        .fontSize(9)
        .fillColor(PDF_COLORS.textSecondary)
        .text(`Notes: ${doctor.notes}`, x + 10, doc.y, {
          width: width - 10,
        });
    }
