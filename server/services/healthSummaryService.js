...
function generateHealthSummary(data) {
  const { bloodPressure, diabetes, bmi, profile } = data;
  ...
  if (diabetes) {
    ...
    if (glucoseValues.length > 0) {
      ...
      if (hasElevatedReading) {
        summary.push(
          "Some of your blood sugar readings are elevated. Limit sugary foods and refined carbohydrates, maintain regular physical activity, and follow your doctor's recommendations.",
        );
      } else {
        summary.push(
          "Your recent blood sugar readings are being monitored. Continue healthy eating habits and regular monitoring.",
        );
      }
    }
  }
  ...
  if (summary.length === 0) {
    return "Complete your health profile and add health records to receive personalized health insights.";
  }
